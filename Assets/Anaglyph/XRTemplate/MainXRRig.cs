...
namespace Anaglyph.XRTemplate
{
	public class MainXRRig : MonoBehaviour
	{
		...
		public static Camera Camera => Instance == null ? UnityEngine.Camera.main : Instance.GetCamera();
		public static Transform TrackingSpace => Instance == null ? null : Instance.GetTrackingSpace();
		...
		{
			if (camera != null && trackingSpace != null)
			{
				return;
			}

			OVRCameraRig ovrRig = GetComponent<OVRCameraRig>();
			if (ovrRig == null)
			{
				ovrRig = FindFirstObjectByType<OVRCameraRig>();
			}
			...
			if (ovrRig != null)
			{
				...
				if (camera == null && ovrRig.centerEyeAnchor != null)
				{
					camera = ovrRig.centerEyeAnchor.GetComponent<Camera>();
				}
			}
